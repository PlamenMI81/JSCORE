...
const APP_KEY = 'kid_SJE-ywynz'
const APP_SECRET = '9f8d9e4ad8c440aea356f69ac5effae1'
...
function registerUser (event) {
  ...
  let password = $('#viewRegister input[name=password]').val()
  let repeatPassword = $('#viewRegister input[name=repeatPass]').val()

  let nameRegex = /[A-Za-z]{5,}/

  if (username.match(nameRegex) && password !== '' && password === repeatPassword) {
    $.ajax({
      method: 'POST',
      url: BASE_URL + 'user/' + APP_KEY + '/',
      headers: AUTH_HEADERS,
      data: JSON.stringify({username: username, password: password, subscriptions:[]}),
...
function loginUser(event) {
  ...
  let password = $('#viewLogin input[name=password]').val()
    $.ajax({
      method: 'POST',
